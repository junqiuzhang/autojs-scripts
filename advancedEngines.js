module.exports = (() => {
  ...
  advancedEngines.execScriptFile = function (path, args, config) {
    let ss = com.stardust.autojs.script.JavaScriptFileSource(
      java.io.File(files.path(path))
    );
  ...
  advancedEngines.execAutoScript = function (path, args, config) {
    let ss = com.stardust.autojs.script.AutoFileSource(
      java.io.File(files.path(path))
    );
    return execute(ss, args, config);
  };
  ...
  var execute = function (scriptSource, args, config) {
    ...
    let listener = ScriptExecutionListener({
      onStart: function (execution) {
    ...
    if (!isPro()) {
      ...
      return runtime.engines.execute(
        null,
        ScriptExecutionTask(scriptSource, listener, fillConfig(config))
      );
  };
  var fillConfig = function (c) {
    ...
    if (isPro())
      config.projectConfig = engines
        .myEngine()
        .getTag('execution.config').projectConfig;
    return config;
  };
  function isPro() {
